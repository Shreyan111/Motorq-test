...
export const updatePost = async (req, res) => {
    ...
    const updatedPost = { creator, title, message, tags, selectedFile, likes, event_capacity, event_start_timestamp, event_end_timestamp, event_location, _id: id };

    await PostMessage.findByIdAndUpdate(id, updatedPost, { new: true, upsert: true });
...
export const registerPost = async(req, res) => {
    ...
    const index = post.event_capacity.findIndex((id) => id === String(req.userId));
    ...
    if (index === -1) {
        ...
        post.event_capacity = post.event_capacity.filter((id) => id !== String(req.userId));
    }
    const updatedPost = await PostMessage.findByIdAndUpdate(id, post, { new: true });
    res.status(200).json(updatedPost);
}
